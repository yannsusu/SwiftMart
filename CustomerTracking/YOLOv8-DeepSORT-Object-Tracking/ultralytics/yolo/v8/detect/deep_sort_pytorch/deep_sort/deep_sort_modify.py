...
class DeepSort(object):
    def __init__(self, model_path, output_dir, max_dist=0.2, min_confidence=0.3, nms_max_overlap=1.0,
    ...
    def update(self, bbox_xywh, confidences, oids, ori_img):
        ...
        # **步骤 1: 先裁剪成正方形 (取中心 720x720)**
        crop_size = min(orig_h, orig_w)  # 取高宽最小值，变成正方形
        crop_y1 = (orig_h - crop_size) // 2  # 上方裁剪的像素数
        crop_y2 = crop_y1 + crop_size  # 下方裁剪的像素数
        cropped_img = ori_img[crop_y1:crop_y2, :]  # 取中间部分，宽度不变

        # **步骤 2: 把裁剪后的 720x720 缩放到 224x224**
        ...
        for track in self.tracker.tracks:
            if not track.is_confirmed() or track.time_since_update > 1:
                continue
            box = track.to_tlwh()
            x1, y1, x2, y2 = self._tlwh_to_xyxy(box, orig_w, crop_y1, crop_y2, target_size)
    ...
    def _get_features(self, bbox_xywh, ori_img):
        ...
        for box in bbox_xywh:
            ...
            # **如果 bbox 完全被裁掉，就跳过**
            if y2 <= 0 or y1 >= ori_img.shape[0]:
                continue

            im = ori_img[max(0, y1):min(ori_img.shape[0], y2), max(0, x1):min(ori_img.shape[1], x2)]
            im_crops.append(im)
    ...
    def _tlwh_to_xyxy(self, bbox_tlwh, img_width, crop_y1, crop_y2, target_size):
        """
        把 tlwh 格式 bbox 转换到 224x224，并修正 Y 轴偏移
        """
        x, y, w, h = bbox_tlwh
        ...
        # **如果 bbox 被完全裁掉（超出上界），设为 0**
        if y2 <= 0:
            return 0, 0, 0, 0

        # **如果 bbox 超出下界（完全消失），设为最大值**
        if y >= crop_y2 - crop_y1:
            return target_size - 1, target_size - 1, target_size - 1, target_size - 1

        # **正常缩放 bbox**
        x1 = max(int(x * target_size / img_width), 0)
        x2 = min(int((x + w) * target_size / img_width), target_size - 1)
        y1 = max(int(y * target_size / (crop_y2 - crop_y1)), 0)
        y2 = min(int(y2 * target_size / (crop_y2 - crop_y1)), target_size - 1)
