...
class DeepSort(object):
    ...
    def update(self, bbox_xywh, confidences, oids, ori_img):
        ...
        for track in self.tracker.tracks:
            ...
            # Save track info and cropped image
            self._save_track_info(track_id, x1, y1, x2, y2, ori_img)

        if len(outputs) > 0:
            outputs = np.stack(outputs, axis=0)
        return outputs

    def _save_track_info(self, track_id, x1, y1, x2, y2, ori_img):
        ...
        img_path = os.path.join(track_dir, f"{track_id}_{self.frame_count}.jpg")
        cv2.imwrite(img_path, cropped_img)

        # Save track info to seq file
        seq_path = os.path.join(track_dir, f"{track_id}.seq")
        with open(seq_path, 'a') as f:
            f.write(f"{self.frame_count},{x1},{y1},{x2},{y2}\n")
